<template>
	<view class="filter-parent-content" v-bind:style="{ maxHeight: height }">
		<view class="filter-item" v-bind:style="{ color: getColor(level), width: getWidth(level) }" :key="itemOut.key"
			v-for="(itemOut, level) in StackList">
			<view class="filter-title" v-if="root.alias !==undefined && root.alias != null  && root.alias.length > 0 ">
				{{root.alias[level]}}<!--{{level}}-->
			</view>
			<view class="filter-item-inner" :key="item.value"
				v-bind:style="[{ backgroundColor: getBackActiveColor(item, level) },{ color: getActiveColor(item, level) }]"
				v-for="item in itemOut.list" @click="selItemFun(item, level)">
				{{ item.text }}
			</view>
		</view>
	</view>
