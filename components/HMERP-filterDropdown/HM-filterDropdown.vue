			...
			<view class="sub-menu-class" :class="{'show':showPage==page_index,'hide':pageState[page_index]!=true}">
				...
				<block v-if="page.type=='radio'">
					<view class="filter">
						...
						<view class="btn-box">
							<!-- <view class="reset" @tap="resetFilterData(page_index)">重置</view> -->
							<view class="submit" style="width: 100%;" @tap="setFilterData(page_index)">确定</view>
						</view>
					</view>
				</block>
			</view>
...
<script>
	...
	export default {
		...
		watch: {
			...
			// 	// console.log('1123',this.defaultActive);
			// 	// console.log('1123',this.activeMenuArr);
			// 	// console.log('1123',this.shadowActiveMenuArr);
		...
		methods: {
			...
			loadCkData() {
				...
				if (data) {
					...
					that.NewfilterData[0].submenu = filedata;
				}
			},
			// 加载分类筛选
			loadcategray(type = -1, label_index) {
				// 0分类 1品牌 2系列 3机型
				let that = this;
				let data = this.filterbasicInfo.category;
				let index = label_index;
				let checkdataList = this.shadowActiveMenuArr;
				let checkdata = [];//选中值
				console.log('上一次选中值',checkdataList);
				if (type === -1) {
					let info = {
						"name": "分类",
						"submenu": []
					...
					info.submenu = filedata;
					that.NewfilterData[1].submenu[0] = info;
				}
				if (type === 0) {
					let info = {
						"name": "品牌",
					...
					let filedata = data[index].children.map((item) => {
						let info1 = {
							name: item.name,
							value: 'brand-' + item.id,
						}
					...
					info.submenu = filedata;
					that.NewfilterData[1].submenu[1] = info;
					that.NewfilterData[1].submenu[2] = [];
					that.NewfilterData[1].submenu[3] = [];
					checkdata = [
						[null,null],
						[[index],[null]]
					]
					// that.reloadcheck(checkdata);
					this.NewfilterData[1].submenu[0].submenu.map((item,index)=>{
						item.selected = false;
					})
					this.NewfilterData[1].submenu[0].submenu[index].selected = true;
				}
				// console.log('选中记录->', this.shadowActiveMenuArr);
				if (type === 1) {
					let info = {
						"name": "系列",
						"submenu": []
					};
					// console.log('12333',checkdataList[1]);
					// console.log(data[checkdataList[1][0][0]].children[index].children);
					// return;
					let filedata = data[checkdataList[1][0][0]].children[index].children.map((item) => {
						let info1 = {
							name: item.name,
							value: 'xilie-' + item.id,
						}
					...
					// console.log(filedata);
					info.submenu = filedata;
					that.NewfilterData[1].submenu[2] = info;
					that.NewfilterData[1].submenu[3] = [];
					checkdata = [
						[null,null],
						[[checkdataList[1][0][0]],[index],[null]]
					]
					this.NewfilterData[1].submenu[0].submenu.map((item,index)=>{
						item.selected = false;
					})
					this.NewfilterData[1].submenu[1].submenu.map((item,index)=>{
						item.selected = false;
					})
					this.NewfilterData[1].submenu[0].submenu[[checkdataList[1][0][0]]].selected = true;
					this.NewfilterData[1].submenu[1].submenu[index].selected = true;
					// that.reloadcheck(checkdata);
				}
				if (type === 2) {
					// return;
					...
					let filedata =  data[checkdataList[1][0][0]].children[checkdataList[1][1][0]].children[index].children.map((item) => {
						let info1 = {
							name: item.name,
							value: 'jixian-' + item.id,
						}
						return info1
					})
					// console.log(filedata);
					info.submenu = filedata;
					that.NewfilterData[1].submenu[3] = info;
					checkdata = [
						[null,null],
						[[checkdataList[1][0][0]],[checkdataList[1][1][0]],[index],[null]]
					]
					// that.reloadcheck(checkdata);
					this.NewfilterData[1].submenu[0].submenu.map((item,index)=>{
						item.selected = false;
					...
					this.NewfilterData[1].submenu[2].submenu.map((item,index)=>{
						item.selected = false;
					})
					this.NewfilterData[1].submenu[0].submenu[checkdataList[1][0][0]].selected = true;
					this.NewfilterData[1].submenu[1].submenu[checkdataList[1][1][0]].selected = true;
					this.NewfilterData[1].submenu[2].submenu[index].selected = true;
				}
				console.log('筛选数据->', this.NewfilterData);
				this.initdata();
				// this.updateMenuName = true;
				// setTimeout(()=>{},1000)
				this.reloadcheck(checkdata);
				
			},
			// 初始化页面
			initdata() {
				this.menuData = JSON.parse(JSON.stringify(this.NewfilterData));
				console.log(this.menuData);
				this.initMenu(); //filterData重新赋值初始化菜单
			},
			// 重置选中
			...
			initMenu() {
				...
				// console.log('123a-',this.activeMenuArr);
			...
			selectRadioLabel(page_index, box_index, label_index) {
				...
				this.loadcategray(box_index, label_index);
