<template>
	<view>
		...
		<bar-title bgColor="bg-white" isBack>
			...
			<block slot="right">
				<view @click="jump_coupon" class="bg-deepblue cu-btn radius-4 text-sm">我的优惠券</view>
			</block>
		</bar-title>
		<!--列表-->
		<view class="block">
			<view class="coupon-sponsored-card-view" v-for="(item,index) in dataList" :key="index">
				<view class="card-price-view">
					...
					<view class="btn-right-view">
						<view class="cu-btn bg-deepblue" @click="RadioChange(item.id)">领取</view>
					</view>
				</view>
				<view class="card-num-view flex">
					<view class="text-xs">剩余{{item.remain_count}}张</view>
				</view>
			</view>
	...
	export default {
		...
		methods: {
			...
			getDataList() {
				...
				usercouponlist(paramsData).then(res => {
