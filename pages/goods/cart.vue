	...
	<view class="container">
		...
		<view v-if="(!hasLogin || empty===true) && state != 'load'" class="empty">
			...
			<view v-if="hasLogin" class="empty-tips">
				空空如也
				<navigator class="navigator" v-if="hasLogin" url="/pages/tabbar/home" open-type="switchTab">随便逛逛></navigator>
			</view>
				...
				<block v-for="(item, index) in cartList" :key="item.cart_id">
					...
					>
						...
						<view class="item-right">
							...
							<uni-number-box class="step" :min="1" :max="Number(item.stock)" :disabled="item.number>=item.stock" :value="cartList[index].number"
	...
	export default {
		...
		methods: {
			...
			async check(type, index) {
				...
				//远程处理
				CartChooseChange({trueArr,falseArr}).then(res=>{
					let result = res.data;
					if (!result) {
						//恢复原来勾选的状态
						list.forEach(item => {
			...
			async numberChange(data) {
				...
				CartNumberChange({'id':cart_id,number:newNumber}).then(res=>{
					let result = res.data;
					if (!result) {
						that.cartList[data.index].number = oldNumber;
						that.calcTotal();
					}
				})
