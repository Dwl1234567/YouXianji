<template>
	<view>
		...
		<view class="bg-white  margin-top padding-bottom-sm padding-top-sm">
			<view class="margin-top-lg cu-card article">
					...
					<view class="content">
						...
						<view class="desc">
							<view class="text-content">
								<view class="text">
									收:<text class="text-red">{{item.money}}</text>元
									调:<text class="text-red">{{item.peer_price}}</text>元
									销:<text class="text-red">{{item.sales_price}}</text>元
								</view>
								<view class="text">SN：{{item.sn}} <text class="margin-left-sm cuIcon-copy text-orange"
										@tap="copy(item.sn)">复制</text></view>
								<view class="text">采购时间：{{item.createtime}}</view>
							</view>
						</view>
						<view class="action">
							<view class="cu-btn bg-green sm margin-top-xs flex align-center"
						...
						<block v-for="(item,index) in nav_list" :key="index">
							<scroll-view v-show="tab_cur == index" scroll-y="true" class="scroll-Y">
								...
								<view class="recy-item" v-for="(citem,cindex) in item.child" :key="cindex">
									...
									<view class='text-left'><input class="text-sm" placeholder="请输入备注信息" v-model="citem.remark" name="input"></input></view>
								</view>
