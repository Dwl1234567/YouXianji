<template>
	<view>
		...
		<!-- 日期选择控件 -->
		<u-calendar :show="showdate" mode="range" :minDate="minDate[0]" :maxDate="maxDate[0]" defaultDate="[]" :allowSameDay="true" :closeOnClicOverlay="true" @confirm="confirmDate"
		...
		<view class="cu-modal drawer-modal justify-start" :class="modalName=='DrawerModalL'?'show':''" @tap="hideModal">
			<view class="cu-dialog" @tap.stop="">
				<view class="cu-list menu text-left">
					...
					<scroll-view class="VerticalNav nav" scroll-y scroll-with-animation :scroll-top="verticalNavTop" style="height:calc(100vh - 305upx)">
		...
		<view class="margin-sm">
			...
			<scroll-view scroll-x class="nav  text-center" scroll-with-animation :scroll-left="zLeft">
				<view class="cu-item text-lg" :class="index==zCur?'text-red text-bold cur':''" v-for="(item,index) in navList" :key="index" @tap="Select" :data-id="index">
					{{item}}
				</view>
			</scroll-view>	
			...
			<view class="" v-if="zCur == 0">
				<view class="padding-tb-sm bg-white radius-3">
					...
					<scroll-view scroll-x class="nav  text-center" scroll-with-animation :scroll-left="sellLeft">
						<view class="cu-item" :class="index==sellCur?'text-red cur':''" v-for="(item,index) in sellList" :key="index" @tap="sellSelect" :data-id="index">
							{{item}}
						</view>
					</scroll-view>	
					<!--三级筛选维度-->
					...
					<block v-if="sellCur==1">
						<view class="flex justify-center">
							<view class="padding-lr-sm ">
								<picker @change="employeeChange" :value="employeekey" :range="xiaoshou">
									<view class="picker">
										{{employeekey>-1?xiaoshou[employeekey]:'销售员'}}
										<text class="iconfont icon-jiantouxiangxia"></text>
									</view>
								</picker>
							</view>
					...
					<block v-if="sellCur==2">
						<view class="flex justify-center">
							<view class="padding-lr-sm">
								<picker @change="salesClientChange" :value="sales_client_key" :range="client">
									<view class="picker">
										{{sales_client_key>-1?client[sales_client_key]:'客户'}}
										<text class="iconfont icon-jiantouxiangxia"></text>
									</view>
								</picker>
							</view>
					...
					<block v-if="sellCur==3">
						<view style="position: relative;">
							<LiFilter :isType="0" :alias="['分类','品牌','系列','机型']" @change="changebar" @select="selectbar" :datalist="filterbasiclist" :height="0.6" :isFixtop="false"></LiFilter>
						</view>
					</block>
				</view>
			...
			<view class="" v-if="zCur == 1">
				<view class="padding-tb-sm bg-white radius-3">
					...
					<scroll-view scroll-x class="nav  text-center" scroll-with-animation :scroll-left="buyLeft">
						<view class="cu-item" :class="index==buyCur?'text-red cur':''" v-for="(item,index) in buyList" :key="index" @tap="buySelect" :data-id="index">
							{{item}}
						</view>
					</scroll-view>	
					<!--三级筛选维度-->
						...
						<view class="flex justify-center">
							<view class="padding-lr-sm ">
								<picker @change="recycleChanneChange" :value="recycle_channel_key" :range="kehu">
									<view class="picker">
										{{recycle_channel_key>-1?kehu[recycle_channel_key]:'线上/下'}}
										<text class="iconfont icon-jiantouxiangxia"></text>
									</view>
								</picker>
							</view>
							...
							<view class="padding-lr-sm">
								<picker @change="recycleClientIdChange" :value="recycle_client0_key" :range="xiaoshou">
									<view class="picker">
										{{recycle_client0_key>-1?xiaoshou[recycle_client0_key]:'客户'}}
										<text class="iconfont icon-jiantouxiangxia"></text>
									</view>
								</picker>
							</view>
					...
					<block v-if="buyCur==1">
						<view class="flex justify-center">
							<view class="padding-lr-sm">
								<picker @change="recyclePeopleChange" :value="recycle_people_key" :range="xiaoshou">
									<view class="picker">
										{{recycle_people_key>-1?xiaoshou[recycle_people_key]:'回收人'}}
										<text class="iconfont icon-jiantouxiangxia"></text>
									</view>
								</picker>
							</view>
					...
					<block v-if="buyCur==2">
						<view class="flex justify-center">
							<view class="padding-lr-sm">
								<picker @change="recycleClientChange" :value="recycle_client_key" :range="client">
									<view class="picker">
										{{recycle_client_key>-1?client[recycle_client_key]:'客户'}}
										<text class="iconfont icon-jiantouxiangxia"></text>
									</view>
								</picker>
							</view>
					...
					<block v-if="buyCur==3">
						<view style="position: relative;">
							<LiFilter :isType="0" :alias="['分类','品牌','系列','机型']"  @select="recycleSelectBar" :datalist="filterbasiclist" :height="1" :isFixtop="false"></LiFilter>
						</view>
					</block>
				</view>
			</view>
...
<script>
	...
	export default {
		//name: 'sales',
		components: {barTitle,LiFilter,sell0,sell1,sell2,sell3,buy0,buy1,buy2,buy3},
		data() {
			return {
				salesTotalData:{},
				recycleTotalData:{},
		...
		methods: {
			...
			GetSalesTotalData(){
				...
				queryparams.type=1;
				erpStatisticGetTotalData(queryparams).then(res =>{
					this.salesTotalData = res.data.sell_data;
				});
			},
			//获取回收统计
			GetRecycleTotalData(){
				...
				queryparams.type=2;
				erpStatisticGetTotalData(queryparams).then(res =>{
					this.recycleTotalData = res.data.recycle_data;
				});
			},
			//获取基础数据
			getSelectData(){
				let queryData = {}
				erpStatisticGetData(queryData).then(res =>{
					...
					if(this.employeeList.length>0){
						for(let i=0;i<this.employeeList.length;i++){
							this.xiaoshou.push(this.employeeList[i]['name']);
						}
					}
					if(this.clientList.length>0){
						for(let i=0;i<this.clientList.length;i++){
							this.client.push(this.clientList[i]['username']);
						}
					}
				});
			},
			...
			//销售员汇总  -- 销售员
			employeeChange(e) {
				this.employeekey = e.detail.value;
			...
			//按客户汇总 -- 客户
			salesClientChange(e){
				this.sales_client_key = e.detail.value;
			...
			recyclePeopleChange(e){
				this.recycle_people_key = e.detail.value;
			...
			recycleClientChange(e){
				this.recycle_client_key= e.detail.value;
