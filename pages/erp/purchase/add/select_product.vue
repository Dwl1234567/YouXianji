			...
			*/
			...
			goodsclick(value) {
				...
				uni.redirectTo({
					...
					url: '/pages/erp/purchase/add/select_recycleinfo?id=' + value.id +'&cate_id='+value.cate_id
				})
			},
			TabSelect(id,index) {
				this.tabCur = index;
			...
			divFormTap() {
				//console.log(111)
				uni.navigateTo({
					url: "/pages/erp/purchase/add/select_diyattr"
				});
			}
		}
	}
...
<style lang="scss" scoped>
	.sell-box {
		...
		.head-box{
			...
			z-index: 9999;
			width: 100%;
			//top: calc(var(--status-bar-height) + 140.09rpx);
			/* #ifdef APP-PLUS */
		...
		.search-box {
			...
			width: 100%;
			/* #ifdef APP-PLUS */
			//padding:calc(var(--status-bar-height) + 20.09rpx) 9.09rpx 0 9.09rpx;
			/* #endif */
