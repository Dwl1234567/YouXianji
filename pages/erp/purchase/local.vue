			...
			<view class="bg-white arrow margin-bottom-xs radius-4 tasklocal" v-for="(item,index) in dataList" :key="index">
				...
				<view class="">
					<view class="flex padding-lr-sm justify-between">
						...
						<view class="fr">
							<view class="cu-btn bg-undo-btn radius-3 sm text-bold" style="margin-right: 20rpx;"  @tap="undoTap(item.id)">取消</view>
							<view class="cu-btn bg-deepblue radius-3 sm text-bold"  @tap="receiptTap(item.id)">回单处理</view>
						</view>
					</view>
...
<script>
	import {
		getusertakerlist,undotakerrecycleorder
	} from "@/api/erpapi.js";
	...
	export default {
		...
		methods: {
			...
			getDataList() {
				...
				getusertakerlist(paramsData).then(res => {
				...
				uni.showModal({
					...
					success: res => {
						if (res.confirm) {
							...
							undotakerrecycleorder(params).then(res => {
								if (res.code == 1) {
									uni.$u.toast(res.msg);
									setTimeout(() => {
										uni.startPullDownRefresh();
			...
			receiptTap(id){
				uni.navigateTo({
					url: "/pages/erp/purchase/local_receipt?Id="+id
				});
			},
			pingzhengTap(e){
