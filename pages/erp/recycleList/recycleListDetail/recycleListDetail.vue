      ...
      <view class="group_2 flex-col">
        ...
        <view class="box_2 flex-col">
          ...
          <view class="text-wrapper_8 flex-row justify-between">
            ...
            <text class="text_24">{{recycleOrder.postType == 0 ? '顺丰上门' : recycleOrder.postType == 1? '自行邮寄' : '同城上门'}}</text>
          </view>
          <view class="text-wrapper_9 flex-row justify-between">
            <text class="text_25">取件地址</text>
            <text class="text_26" v-if="recycleOrder.pickUpAddressObj">{{recycleOrder.pickUpAddressObj.provinceName}},{{recycleOrder.pickUpAddressObj.cityName}},{{recycleOrder.pickUpAddressObj.regionName}},{{recycleOrder.pickUpAddressObj.detailAddress}}</text>
          </view>
          <view class="group_7 flex-row justify-between">
            <text class="text_27">收款账户</text>
			<view class="flex-row">
				<text class="text_28" v-if="recycleOrder.userAccount">{{recycleOrder.userAccount.bankName}}{{recycleOrder.userAccount.accountNo}}</text>
        ...
        <view class="box_8 flex-col">
		  ...
		  <view class="text-wrapper_15 flex-row justify-between" v-for="item in qualityInfoList" v-if="item.indexs == 1 && tab === 1">
			<text class="text_39">{{item.key}}</text>
			<text class="text_40">{{item.value}}</text>
		  </view>
		   <view class="text-wrapper_15 flex-row justify-between" v-for="item in qualityInfoList" v-if="item.indexs == 2 && tab === 2">
		   	<text class="text_39">{{item.key}}</text>
		   	<text class="text_40">{{item.value}}</text>
		   </view> 
		   <view class="text-wrapper_15 flex-row justify-between" v-for="item in qualityInfoList" v-if="item.indexs == 3 && tab === 3">
		   	<text class="text_39">{{item.key}}</text>
		   	<text class="text_40">{{item.value}}</text>
		   </view> 
...
<script>
import { empSelectRecycleOrderDetail } from '@/api/erp.js';
export default {
  data() {
    return {
	  // 员工id
