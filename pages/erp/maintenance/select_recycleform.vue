...
<script>
	...
	export default {
		...
		onLoad(options) {
			...
			this.getInfoByRecycleOrderId(options.recycleOrderId)
		},
		watch: {

			xiaoshoujianum() {
				console.log(this.xiaoshoujianum)
		...
		methods: {
			...
			erpclickattreditFuc() {
				...
				let paramsData = {
					reorganizeId: Number(this.recycleOrderId),
				...
				reoragnizePutaway(paramsData).then(res => {
					if (res.code === 200) {
						uni.showToast({
							icon: 'none',
							title: '修改成功',
