...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			this.recycleOrderId = option.reorganizeId;
			this.goodsInfo = option;
			this.goodsId = option.modelId;
			this.modelName = option.modelName;
			this.type = option.type;
			this.getInfoByRecycleOrderId(option.reorganizeId);
		...
		methods: {
			getInfoByRecycleOrderId(recycleOrderId) {
				getPutawayInfo(recycleOrderId).then((res) => {
			...
			async deliveryTap() {
				uni.navigateTo({
					url: '/pages/erp/maintenance/select_recycleform?forecastMoney=' +
						this.forecastMoney +
						'&recycleOrderId=' +
						this.recycleOrderId,
				});
