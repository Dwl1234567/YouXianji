		...
		<view class="head-box">
			...
			<view class="search-box flex">
				...
				<view class="diybtn margin-tb-sm text-center" @tap="divFormTap">
					<view class="cu-btn round bg-red">未上架机型点这里</view>
				</view>
			</view>
			<!-- 搜索联想 -->
			...
			<view class="rootcate-box">
				<scroll-view scroll-x class="bg-white nav" scroll-with-animation :scroll-left="scrollLeft">
					<view
						class="cu-item text-bold text-lg"
						:class="index==topCur?'text-red cur':''"
						v-for="(item,index) in dataListArr"
						:key="index"
						@tap="rootcateclick(item.classificationId,index)"
						:data-id="item.classificationId,index"
					>
		...
		<view class="VerticalBox" v-if="goodsDataArr">
			...
			>
				<view
					...
					v-for="(item,index) in goodsDataArr"
					:key="index"
				...
				<view class="categoods-box">
					<u-grid :border="false">
						<u-grid-item
							...
							:key="baseListIndex"
							@click="goodsclick(baseListItem)"
							v-if="goodsDataArr[tabCur]"
						>
...
<script>
	import { HTTP_REQUEST_IMAGEURL } from '@/config/app';
	import { Godscategory, GodsgoodsList, searchRecycleData } from '@/api/common.js';
	import { getSeries, getAllClassification } from '@/api/retrieve.js';
	...
	export default {
		...
		methods: {
			...
			goodsclick(value) {
				...
				uni.navigateTo({
					url:
						'/pages/tabbarerp/recycling-new?id=' +
						value.modelId +
						'&modelName=' +
						value.modelName +
						'&photo=' +
