		...
		methods: {
			...
			async deleteOrder(index) {
				...
				if (res.confirm) { 
					...
					OrderDelete({order_id:order_id}).then(res=>{
						let result = res.data;
						if (result) {
			...
			async cancelOrder(item) {
				...
				if (res.confirm) {
					...
					OrderCancel({order_id:item.order_id}).then(res=>{
						let result = res.data;
						if (result) {
							let {
								stateTip,
			...
			async receivedOrder(item) {
				...
				if (res.confirm == true) {
					...
					// });
					OrderReceived({order_id: item.order_id}).then(res=>{
						let result = res.data;
						if (res) {
							// 已确认收货
			...
			button(action, item) {
				switch (action) {
					case 'cancel':
						...
						break;
					case 'pay':
						this.navTo('/pages/goods/pay?order_id=' + item.order_id + '&total=' + item.total_price);
						break;
					case 'delivery':
						// this.navTo('/pages/goods/express?expresssn=' + item.extend.express_number + '&express=' + item.extend.express_company);
						console.log('等待接入');
						break;
					case 'recerved':
						...
						break;
					case 'evaluate':
						this.$api.navTo('/pages/goods/evaluate?product_id='+item.id+'&order_id='+item.order_id+'&image='+item.image+'&title='+item.title+'&spec='+item.spec);
						break;
					case 'refund':
						this.$api.navTo('/pages/goods/refund?order_id=' + item.order_id);
						break;
				}
			},
			pullDownRefresh() {
