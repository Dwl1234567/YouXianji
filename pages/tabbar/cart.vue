<template>
	<view class="cart-box container">
		...
		<view class="cart-list-view">
			<!-- 列表 -->
			<view class="cart-list bg-white margin-sm radius-4">
				<block v-for="(item, index) in cartList" :key="item.cart_id">
					...
					>
						<view class="image-wrapper">
							<image :src="item.image" class="loaded radius-4" mode="aspectFill"></image>
							<view v-if="item.isset == true" class=" iconfont icon-circleorange checkbox" :class="{checked: item.choose}" @click.stop="check('item', index)"></view>
						</view>
						<view class="item-right">
							<text class="clamp title">{{item.title}}</text>
							<text class="attr" v-if="item.spec">{{item.spec}}</text>
							<view class="price margin-top-xs">
								￥<text class="text-red text-xxl  text-bold">{{item.nowPrice}} </text>
		...
		<view class="action-section bg-white" v-if="state != 'load'">
			...
			<button class="no-border cu-btn bg-deepblue radius-4" @click="createOrder">结算</button>
		</view>
	...
	export default {
		...
		methods: {
			...
			async numberChange(data) {
				...
				// 	number: newNumber
				// }, false);
				CartNumberChange({id:cart_id,number: newNumber}).then(res=>{
...
<style lang="scss">
	...
	.cart-item {
		display: flex;
		position: relative;
		padding: 30upx 20upx 30upx 80upx;
		.clamp{
